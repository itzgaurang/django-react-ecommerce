...
def generate_session_token(length=10):
    return ''.join(random.SystemRandom().choice([chr(i) for i in range(97, 13)] + [str(i) for i in range(10)]) for _ in range(length))
...
def signout(request, id):
    ...
    return JsonResponse({'succes': 'Logged Out Successfully'})
